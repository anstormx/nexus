...
import SwapV1 from "../../utils/multiTokenSwapV1.json";
import liquidity from "../../utils/liquidityProvider.json";
import v3pool from "../../utils/uniswapV3Pool.json";
...
export default function Swap() {
	...
	// Add new useEffect for fetching balances when account changes
	useEffect(() => {
	...
	const fetchPrice = useCallback(
		async (inputToken, outputToken, inputAmount) => {
			...
			try {
				...
				const outputTokenAddress = ethers.getAddress(
					outputToken.address
				);
				...
				// The price from slot0 is already in token1/token0 format
				// We need to determine if we need to invert it based on token order
				const isInputToken0 =
					inputTokenAddress.toLowerCase() <
					outputTokenAddress.toLowerCase();
				const adjustedPrice = isInputToken0 ? price : 1 / price;

				// Calculate the output amount with proper decimal handling
				const inputAmountDecimal = parseFloat(inputAmount);
				const outputAmount = inputAmountDecimal * adjustedPrice;

				// Format output amount to 6 decimal places
		...
		try {
			...
			for (let i = 0; i < inputTokens.length; i++) {
				...
				try {
					...
					toast.success(
						`Approved spending of ${inputTokens[i].token.symbol}`
					);
				} catch (error) {
					console.log(
...
function InputBox({
	...
	const handleInputValidation = (e) => {
		...
		// Must start with a number, not a decimal point
